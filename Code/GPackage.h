...
#define		_GPACKAGE_STATIC(n,u)		static GPackage _GPACKAGE_ ## u ## _NAME(n);
...
public:
	...
	/// Opens a package for reading from the resources and adds itself to the global list of packages
	bool OpenForRead (const GString& path);
	
	/// Opens a package for writing (not a resource but actual file)
	bool OpenFileForWrite (const GString& path);
