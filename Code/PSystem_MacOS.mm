...
- (void) mouseDown: (NSEvent*)theEvent {
	...
	PSystem::RunMouseCallbacks((int_t)location.x, (int_t)location.y, (int_t)[theEvent buttonNumber]);
}
...
- (void) mouseUp: (NSEvent*)theEvent {
	NSPoint location = [self convertPoint:[theEvent locationInWindow] fromView:nil];
	location.y = self.frame.size.height - location.y;
	PSystem::RunMouseUpCallbacks((int_t)location.x, (int_t)location.y, (int_t)[theEvent buttonNumber]);
}
...
- (void) mouseMoved: (NSEvent*)theEvent {
	NSPoint location = [self convertPoint:[theEvent locationInWindow] fromView:nil];
	location.y = self.frame.size.height - location.y;
	PSystem::RunMouseMoveCallbacks((int_t)location.x, (int_t)location.y);
}

- (void) mouseDragged: (NSEvent*)theEvent {
	NSPoint location = [self convertPoint:[theEvent locationInWindow] fromView:nil];
	location.y = self.frame.size.height - location.y;
	PSystem::RunMouseDragCallbacks((int_t)location.x, (int_t)location.y, (int_t)[theEvent buttonNumber]);
}
...
- (void) scrollWheel: (NSEvent*)theEvent {
	PSystem::RunMouseWheelCallbacks((float_t)[theEvent deltaX], (float_t)[theEvent deltaY]);
}

- (void) keyDown: (NSEvent*)theEvent {
	if([theEvent isARepeat] == NO)
		PSystem::RunKeyCallbacks((vkey_t)[theEvent keyCode]);
	if([[theEvent characters] length] > 0 && isprint((int)[[theEvent characters] UTF8String][0]))
		PSystem::RunASCIICallbacks((char)[[theEvent characters] UTF8String][0]);
...
- (void) keyUp: (NSEvent*)theEvent {
	if([theEvent isARepeat] == NO)
		PSystem::RunKeyUpCallbacks((vkey_t)[theEvent keyCode]);
}
